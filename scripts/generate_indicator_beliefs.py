...
def build_reference_section(rows: list[dict[str, str]], catalog: dict[str, dict[str, str]]) -> list[str]:
    reference_lines = [
        "// ----------------------------------------------",
    ...
    for row in rows:
        event_id = resolve_event_id(row, catalog, "reference CSV")
        reference_lines.extend(
            [
                f"reference_hazard({event_id}, {row['reference_hazard']}).",
                f"reference_severity({event_id}, {row['reference_severity']}).",
                f"reference_confidence({event_id}, {row['reference_confidence']}).",
                "",
            ]
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--reference-csv",
        default="data/processed/reference/reference_labels.csv",
        help="Reference labels CSV path.",
    )
    parser.add_argument(
        "--events-asl",
    ...
    parser.add_argument(
        "--reference-out",
        default="mas/beliefs/generated/reference_labels.asl",
        help="Output path for reference label beliefs.",
    )
    ...
    reference_out = resolve_project_path(args.reference_out)
    ...
    reference_lines = build_reference_section(reference_rows, catalog)
