...
+!dispatch_events([E|Rest]) <-
    ...
    .send(Agent, achieve, assess_event(E, coordinator_agent));
...
+explanation_artifact(E, Headline, AssessmentSentence, FusionSentence, CaveatSentence, ExplanationTrace)[source(explanation_agent)] :
    ...
    .print("-----");
    .print("Event: ", E);
    .print("Integrated symbolic case: hazard=", Hazard, ", severity=", Severity, ", claim=", ClaimLabel, ", exposure=", ExposureClass, ", concern=", ConcernLevel, ", fusion_confidence=", FusionConfidence, ", interpretation=", InterpretationMode, ", profile=", Profile, ", source_agent=", SourceAgent);
    .print("Explanation trace: ", ExplanationTrace);
    .print(Headline);
    .print(AssessmentSentence);
    .print(FusionSentence);
    .print(CaveatSentence);
    !report_reference_alignment(E, Hazard, Severity, FusionConfidence).
...
+!report_reference_alignment(E, Hazard, Severity, FusionConfidence) <-
    ...
    .print("Reference alignment for ", E, ": hazard=", HazardMatch, ", severity=", SeverityMatch, ", confidence=", ConfidenceMatch).
