...
Figure~\ref{fig:system_overview_diagram} provides a compact interaction view of the overall pipeline, from the initial EO belief loading stage to symbolic reasoning, explanation packaging, and final NLP post-processing. This figure is useful as a bridge between the conceptual three-layer architecture and the more detailed reasoning diagrams discussed later.

\begin{figure}[H]
    \centering
    \fitdiagram{figures/diagram.pdf}
    \caption{System-level interaction view of the EO2Explain pipeline. The diagram shows how EO beliefs are dispatched to the coordinator, how the WaterAgent produces a first-pass symbolic assessment with optional clarification, how the ExplanationAgent packages the semantic explanation, and how the NLP layer transforms that payload into a final report.}
...
Figure~\ref{fig:sequence_diagram} summarizes the temporal communication flow of the symbolic multi-agent system in a compact form. The CoordinatorAgent acts as the orchestration point: it dispatches event beliefs to a hazard-specialized agent, receives the resulting assessment, optionally requests clarification when the first pass remains uncertain, and then forwards the integrated symbolic output to the ExplanationAgent. The figure also keeps the downstream NLP handoff visible so that the reasoning layer can be understood in relation to the post-processing stage.

\begin{figure}[H]
    \centering
    \fitdiagram{figures/diagram.pdf}
    \caption{System-level interaction view of the reasoning and reporting flow. The coordinator dispatches the event to the WaterAgent, may request clarification, then sends the integrated symbolic output to the ExplanationAgent and finally exposes the semantic payload to the NLP layer for report generation.}
...
\begin{figure}[H]
    ...
    \caption{Detailed communication and reasoning flow for the \texttt{water\_agent}. The diagram shows how EO and context beliefs are turned into indicator-level signals, higher-level evidence patterns, support-level assignments, severity paths, caveat profiles, and optional clarification outputs before returning a structured \texttt{hazard\_assessment(...)} to the coordinator.}
