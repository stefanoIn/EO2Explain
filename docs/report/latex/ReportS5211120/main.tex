...
The EO preprocessing stage is implemented through Python scripts:
...
\node[agent] (water) at (-4.8,-2.5) {WaterAgent};
\node[agent] (forest) at (-4.8, -3.5) {ForestAgent};
\node[agent] (expl) at (0,-5.3) {ExplanationAgent};
\node[layer] (python) at (0,-7.9) {Python semantic layer\\transformer, ontology, NLG};

\draw[flow] (coord.south west) -- (water.north east)
    node[pos=0.24, above left, msg] {assess\_event}
    node[pos=0.74, below right, msg] {clarify\_assessment};

\draw[flow] (water.north east) -- (coord.south west)
    node[pos=0.34, below right, msg] {hazard\_assessment}
    node[pos=0.80, above left, msg] {clarification\_result};

\draw[flow] (coord.south east) -- (forest.north west)
    node[pos=0.24, above right, msg] {assess\_event}
    node[pos=0.74, below left, msg] {clarify\_assessment};

\draw[flow] (forest.north west) -- (coord.south east)
    node[pos=0.34, below left, msg] {hazard\_assessment}
    node[pos=0.80, above right, msg] {clarification\_result};
...
The same coordination logic can also be read as a temporal interaction sequence. Figure~\ref{fig:mas_sequence_diagram} shows the order in which the coordinator gathers hazard assessments, opens an optional clarification round for uncertain cases, requests explanation construction, and then hands the exported semantic payload to the downstream Python layer.

\begin{figure}[H]
\centering
\resizebox{\textwidth}{!}{
\begin{sequencediagram}
    \newthread{coord}{}{CoordinatorAgent}
    \newinst{water}{}{WaterAgent}
    \newinst{forest}{}{ForestAgent}
    \newinst{expl}{}{ExplanationAgent}
    \newinst{py}{}{Python Layer}

    \begin{call}{coord}{assess\_event(E)}{water}{hazard\_assessment}
    \end{call}
    \begin{call}{coord}{assess\_event(E)}{forest}{hazard\_assessment}
    \end{call}

    \begin{sdloop}{If uncertainty or caveats require refinement}
        \begin{call}{coord}{clarify\_assessment(E,\dots)}{water}{clarification\_result}
        \end{call}
        \begin{call}{coord}{clarify\_assessment(E,\dots)}{forest}{clarification\_result}
        \end{call}
    \end{sdloop}

    \begin{call}{coord}{build\_explanation(E)}{expl}{semantic\_explanation}
    \end{call}
    \begin{call}{expl}{export\_semantic\_payload()}{py}{JSON / ontology / report}
    \end{call}
\end{sequencediagram}
}
\caption{Sequence view of the EO2Explain multi-agent workflow. The coordinator collects specialist assessments, optionally requests clarification, and then triggers explanation export for downstream semantic processing.}
\label{fig:mas_sequence_diagram}
\end{figure}
...
The belief base also contains contextual information used during fusion. In particular, each event is associated with a population exposure class derived from WorldPop data. These contextual beliefs are not hazard claims by themselves, but they are used by the coordinator to combine hazard evidence with exposure and derive the final integrated concern profile.
