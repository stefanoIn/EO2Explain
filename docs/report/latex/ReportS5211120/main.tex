...
\begin{tikzpicture}[
    x=1cm,
    y=1cm,
    agent/.style={draw, rounded corners=8pt, thick, fill=blue!6, minimum width=4.1cm, minimum height=1cm, align=center, font=\bfseries},
    layer/.style={draw, rounded corners=8pt, thick, fill=green!8, minimum width=5.4cm, minimum height=1.1cm, align=center},
    flow/.style={->, thick},
    loop/.style={<->, thick}
]
\node[agent] (coord) at (0,0) {CoordinatorAgent};
\node[agent] (water) at (-4.4,-2.4) {WaterAgent};
\node[agent] (forest) at (4.4,-2.4) {ForestAgent};
\node[agent] (expl) at (0,-4.8) {ExplanationAgent};
\node[layer] (python) at (0,-7.2) {Python semantic layer\\transformer, ontology, NLG};

\draw[loop] (coord) -- node[midway, above left, font=\scriptsize, align=center]
{assess\_event\\clarify\_assessment} (water);
\draw[loop] (coord) -- node[midway, above right, font=\scriptsize, align=center]
{assess\_event\\clarify\_assessment} (forest);

\draw[flow] (water) -- node[midway, left, font=\scriptsize, align=center]
{hazard\_assessment\\clarification\_result} (coord);
\draw[flow] (forest) -- node[midway, right, font=\scriptsize, align=center]
{hazard\_assessment\\clarification\_result} (coord);

\draw[flow] (coord) -- node[midway, right, font=\scriptsize]
{build\_explanation} (expl);
\draw[flow] (expl) -- node[midway, right, font=\scriptsize, align=center]
{semantic payload\\JSON export} (python);
\end{tikzpicture}
