...
Hazard interpretation is performed by two specialised agents: the \texttt{forest\_agent} for wildfire cases and the \texttt{water\_agent} for flood cases.
...
From an architectural perspective, the system is designed to be readily extensible. Its modular agent-based structure separates coordination, hazard-specific reasoning, and explanation generation into distinct roles, allowing new hazard-specialized agents to be introduced without modifying the overall communication workflow. In particular, the shared reasoning template adopted by the WaterAgent and ForestAgent illustrates how additional agents could be integrated by reusing the same interaction pattern while adapting only the domain-specific rules and indicators. This makes the architecture well suited for horizontal scaling across hazard types and, in principle, for more complex multi-coordinator hierarchies.

At the same time, scalability remains primarily architectural rather than fully operational. The current design relies on a central CoordinatorAgent that orchestrates all interactions, which may become a bottleneck in larger deployments. Moreover, the rule-based nature of the hazard agents, while essential for explainability, implies that extending the system to additional domains would require careful rule design and maintenance. These considerations highlight a trade-off between transparency and scalability that would need to be addressed in future developments.
