...
\begin{tikzpicture}[
    ...
    family/.style={draw, rounded corners=8pt, thick, fill=gray!10, minimum width=5.6cm, minimum height=0.9cm, align=center, font=\bfseries},
    eventbox/.style={draw, rounded corners=6pt, fill=blue!4, minimum width=5.6cm, minimum height=0.85cm, align=center},
    phase/.style={draw, rounded corners=4pt, fill=white, minimum width=2.45cm, minimum height=0.7cm, align=center, font=\small},
]

\node[family] (wildfires) {Wildfire Cases};
\node[family, right=2.2cm of wildfires] (floods) {Flood Cases};

\node[eventbox, below=0.8cm of wildfires] (greece) {Greece};
\node[eventbox, below=0.35cm of greece] (turkey) {Turkey};
\node[eventbox, below=0.35cm of turkey] (serra) {Serra da Estrela};

\node[eventbox, below=0.8cm of floods] (emilia) {Emilia-Romagna};
\node[eventbox, below=0.35cm of emilia] (ahr) {Ahr Valley};
\node[eventbox, below=0.35cm of ahr] (pakistan) {Pakistan};

\node[phase, below left=0.18cm and 0.02cm of greece] (wb1) {Before image};
\node[phase, below right=0.18cm and 0.02cm of greece] (wa1) {After image};
\node[phase, below left=0.18cm and 0.02cm of emilia] (fb1) {Before image};
\node[phase, below right=0.18cm and 0.02cm of emilia] (fa1) {After image};

\draw[-] (wildfires.south) -- (greece.north);
\draw[-] (wildfires.south) -- (turkey.north);
\draw[-] (wildfires.south) -- (serra.north);
\draw[-] (floods.south) -- (emilia.north);
\draw[-] (floods.south) -- (ahr.north);
\draw[-] (floods.south) -- (pakistan.north);

\draw[-] (greece.south west) -- (wb1.north);
\draw[-] (greece.south east) -- (wa1.north);
\draw[-] (emilia.south west) -- (fb1.north);
\draw[-] (emilia.south east) -- (fa1.north);

\node[below=0.55cm of wb1, align=center, font=\small] (noteleft) {Each case is processed as a\\before/after EO comparison pair};
\node[below=0.55cm of fb1, align=center, font=\small] (noteright) {Indicators are then derived\\from the paired observations};
\end{tikzpicture}
...
\begin{lstlisting}
hazard_assessment(E, Hazard, Severity, ConclusionStatus, ConfidenceLevel,
...
The thresholds used in the hazard agents should be understood as interpretable prototype cutoffs rather than as operational EO standards. In the current implementation, they were selected to produce distinguishable symbolic categories within the small case-study dataset and to support meaningful differences between strong, moderate, weak, conflicting, and inconclusive reasoning paths.

More precisely, the thresholds are intended to:

\begin{itemize}
\item reflect typical relative ranges observed in the EO indicators computed in this project
...
This is important because the reporting layer is not producing free text from scratch. It is verbalizing a structured semantic object that has already been made explicit by the MAS, packaged by the \texttt{explanation\_agent}, and normalized by the transformer. The final English report is therefore grounded in a traceable symbolic source rather than in an opaque language-generation step.
...
\begin{itemize}
\item choose whether the event should be processed as flood or wildfire
\item provide event metadata such as name, location, and temporal-quality assumptions
\item upload the hazard-specific Sentinel-2 single-band GeoTIFFs required by the EO scripts
\item optionally provide a \texttt{user\_assessment} severity expectation
\item inspect the raw report, semantic JSON outputs, and MAS execution trace
\end{itemize}

From an architectural point of view, the UI simply prepares per-job beliefs, launches the EO scripts, runs the MAS, executes the ontology and reporting steps, and then displays the outputs. The reasoning itself remains in Jason. The UI therefore acts as an interaction layer over the EO $\rightarrow$ MAS $\rightarrow$ ontology/report pipeline rather than as an additional reasoning component.
