...
\item a clarification sentence indicating whether second-pass specialist clarification was required
\item an explicit \texttt{explanation\_trace(...)}
\end{itemize}

\begin{lstlisting}
explanation_artifact(E, Headline, AssessmentSentence,
    FusionSentence, CaveatSentence, ClarificationSentence,
    explanation_trace(SourceAgent, RuleLabel, ClaimLabel,
        EvidenceList, CaveatList, clarification_trace(...)))
\end{lstlisting}

This trace is important because it preserves the provenance of the final explanation. The generated text is therefore not a detached summary: it is grounded in the exact symbolic rule that fired, the evidence predicates that supported the claim, the caveats that qualified the interpretation, and the clarification status associated with the case.
