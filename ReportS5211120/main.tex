...
\section{Symbolic Representation of EO Events}

After extracting EO indicators from Sentinel-2 imagery, the results are converted into symbolic knowledge that can be used by the multi-agent system implemented in Jason. The symbolic layer is designed to make the EO evidence explicit and interpretable by reasoning agents.

The knowledge base is divided into two components:

\begin{itemize}
\item an \textbf{event metadata layer}, describing environmental events and their geographic context
\item an \textbf{indicator layer}, containing the EO-derived measurements used for reasoning
\end{itemize}

\subsection{Event Metadata Representation}

Each environmental event is represented as a set of logical facts describing its location, country, event type, and observation dates. This information provides contextual knowledge for the reasoning agents.
...
In addition to the geographic context, each event is associated with a spatial bounding box defining the area used for EO analysis.
...
This spatial information allows the EO indicators to be linked to a specific geographic region.

\subsection{EO Indicator Representation}

The EO indicators computed during the preprocessing stage are also represented as logical facts. These values provide the numerical evidence used by the reasoning agents.

Flood events are described using NDWI-based indicators that capture changes in water extent.
...
Wildfire events are represented using vegetation and burn-related indicators derived from NDVI and NBR.
...
These symbolic facts constitute the belief base of the multi-agent system. Each agent can access the indicators and apply logical rules to determine whether an environmental event corresponds to flooding or wildfire damage.

\subsection{Role in the Multi-Agent System}

The symbolic knowledge base enables reasoning agents to interpret EO indicators through logical rules. For example, a water-related agent may detect flooding when a large increase in water coverage is observed.

\begin{lstlisting}
flood_detected(E) :-
    water_increase_pct(E, W),
    W > 15.
\end{lstlisting}

Similarly, wildfire detection can be based on vegetation loss and burned area indicators.

\begin{lstlisting}
wildfire_detected(E) :-
    vegetation_loss_pct(E, V),
    burned_area_pct(E, B),
    V > 15,
    B > 10.
\end{lstlisting}

By representing EO measurements as logical beliefs, the system can combine satellite evidence with symbolic reasoning and produce structured interpretations that are transparent and explainable.
